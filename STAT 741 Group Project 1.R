install.packages(c("haven", "tidyverse", "lme4", "lmerTest", "sjPlot", "lattice"))
...
# Plot predicted values for Asthma vs Non-Asthma over time
# 'type = "pred"' automatically handles the interaction and controls for other variables at their mean
plot_model(model_rq1, type = "pred", terms = c("age_centered", "asthma")) +
  labs(
    title = "Predicted Rate of Lung Function Decline",
    subtitle = "Adjusted for Sex, Height, and Smoking History",
...
plot_model(model_rq2, type = "pred", terms = c("age_centered", "asthma", "smkever")) +
  labs(
    title = "Lung Function Decline: Asthma x Smoking Interaction",
    y = "Predicted FEV1 (Liters)",
    x = "Years from Mean Age",
    color = "Asthma Status"
  ) +
