...
cleaned_data <- data %>%
  ...
  dplyr::select(id, family, sex, asthma, mwt, mht, smkever, smkever_new, 
                survey, age, age_centered, smk, smk_new, cncig, cncig_new, fev, fvc) 
...
model_rq1_null <- lme(fev ~ asthma * age_centered + smkever + mht + sex,
...
model_rq1_car1 <- lme(fev ~ asthma * age_centered + smkever + mht + sex,
                      random = list(family = ~ 1, id = ~ 1 + age_centered),
...
par(mfrow=c(1,1)) 
qqnorm(re$family$`(Intercept)`, main = "Random Intercept (Family)")
qqline(re$family$`(Intercept)`, col = "red")

# 4. Test for serial correlation
# Extract residuals from model
res <- residuals(model_rq1_car1)
